...
function calc() {
  ...
  var sv2 = document.getElementsByName('sv2')[0]
    && document.getElementsByName('sv2')[0].checked
    && dan.lnote;
  var note = [0], pre = [0], acc = [0]; // 物量, 物量前缀和, 输入 ACC
  for (var i = 1; i <= dan.num; i++) {
    note[i] = dan.note[i - 1] + (sv2 ? dan.lnote[i - 1] : 0)
    pre[i] = pre[i - 1] + note[i];
    acc[i] = Number(document.getElementById('input_' + i).value).toFixed(2);
  }
  var out = '';
  if (mode === 'normal') {
    out = acc.slice(1).join('-') + (sv2 ? ' (Score v2)' : '') + '\n单曲';
    for (var i = 1; i <= dan.num; i++) {
      var single = (acc[i] * pre[i] - acc[i - 1] * pre[i - 1]) / note[i];
      out += ' ' + single.toFixed(2);
    }
  } else {
    out = '单曲 ' + acc.slice(1).join(' ') + (sv2 ? ' (Score v2)' : '') + '\n推算';
...
function inputACC() {
  ...
  for (var i = 1; i <= dan.num; i++) {
    var hint = '第 ' + i + (mode === 'normal' ? ' 首歌结束时的 ACC' : ' 首歌的单曲 ACC');
    html += '<input class="card" type="text" id="input_' + i + '" placeholder="' + hint + '">\n';
  }
  html += '<br>';
  if (dan.lnote) html += '<div><label><input type="checkbox" name="sv2">启用 osu! 中的 Score v2 Mod</label></div>\n';
